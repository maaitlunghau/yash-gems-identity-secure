...
- **Quy tắc ra quyết định tự động chuyển đổi `KycStatus`:**
    - `Verified`: Nếu tỉ lệ khớp khuôn mặt **> 85%** và ảnh rõ nét. AI tự động đổi trạng thái DB thành `Verified`.
    ...
    - `Pending`: Nếu tỉ lệ nằm ở mức lấp lửng **(60% - 85%)** hoặc AI nghi ngờ (ví dụ: ảnh cũ quá, hoặc quá mờ). AI sẽ giữ nguyên trạng thái `Pending` và tạo một cờ (Flag) đẩy hồ sơ này vào hàng đợi để **nhân viên Admin** duyệt tay thủ công.
